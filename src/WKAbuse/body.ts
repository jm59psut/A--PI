...
function WKAbuseF(): void {
	...
	var buff: Buff = MyEnt.BuffByName("modifier_skeleton_king_reincarnation_scepter_active"),
		...
		waitTime = 1 + Corona.MyTick * (bkb === undefined ? 1 : 2)
	if(buff === undefined || tp === undefined || tp.CooldownTimeRemaining > 0 || buff.RemainingTime - (tp.ChannelTime + waitTime) > Corona.MyTick) return
	doing_tp = true
	...
	$.Schedule(waitTime + Corona.MyTick, () => doing_tp = false)
}
...
module = {
	...
	onDestroy: () => Corona.OnTick.remove(WKAbuseF)
}