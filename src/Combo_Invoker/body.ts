...
function Invoke(abil_name: string) {
	combo.addDelay((caster: Entity) => caster.AbilityByName("invoker_invoke").CooldownTimeRemaining + Fusion.MyTick, {
...
function onPreloadF_Combo_Invoker(): void {
	...
	combo.addDelay((caster: Entity, target: Entity) => {
		let eul_buff = target.BuffByName("modifier_eul_cyclone")
		if(eul_buff === undefined) return -1
		return eul_buff.Duration - eul_buff.ElapsedTime - caster.AbilityByName("invoker_chaos_meteor").SpecialValueFor("land_time") + Fusion.MyTick
	}) 
	combo.addAbility("custom_cast", EComboAction.CURSOR_ENEMY, {
		custom_cast: (caster: Entity, target: Entity) => {
			...
			return meteor.CastPoint + Fusion.MyTick
		}
	})
	...
	combo.addDelay((caster: Entity, target: Entity) => {
		...
		return eul_buff.Duration - eul_buff.ElapsedTime - travel_time + Fusion.MyTick * 2
	})
	...
	if(!Fusion.Commands.InvokerCombo) {
		Fusion.Commands.InvokerCombo = () => combo.execute(EntityManager.MyEnt)

		Game.AddCommand("__InvokerCombo", Fusion.Commands.InvokerCombo, "", 0)
	}

	if(!Fusion.Commands.InvokerCombo_AbilsNames) {
		Fusion.Commands.InvokerCombo_AbilsNames = () => combo.abilsNames.forEach(abilName => $.Msg("InvokerCombo", abilName))

		Game.AddCommand("__InvokerCombo_AbilsNames", Fusion.Commands.InvokerCombo_AbilsNames, "", 0)
	}
}
